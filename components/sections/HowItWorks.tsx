...
const STEPS = [
  ...
  },
  {
    ...
    description: "Be among the first in India to get your viral inflatable buddy.",
  },
] as const;
  ...
  return (
    <Section id="how-it-works" className="bg-surface/30">
      ...
      <div className="relative mt-16 grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="pointer-events-none absolute top-7 left-0 hidden h-px w-full bg-gradient-to-r from-transparent via-border to-transparent lg:block" />

        {STEPS.map(({ icon: Icon, title, description }, index) => (
          <FadeIn key={title} delay={index * 0.12} className="relative">
            <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
              <div className="relative z-10 mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-primary/30 bg-background text-primary shadow-lg shadow-primary/10">
                <Icon className="h-6 w-6" />
                <span className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-primary to-secondary font-heading text-xs font-bold text-white">
                  {index + 1}
                </span>
              </div>
